...
void callAgent(HTTPServerRequest req, HTTPServerResponse res){
  ...
  if (commands.length==0){
    res.writeBody("commands needed,using ?commands=some commands in query string", 400);
  } else {
    import std.stdio;
    auto client = req.peer();
    if ( client.startsWith("127.0.0.1") || client.startsWith("[0:0:0:0:0:0:0:1]")){
      ...
      res.writeBody("service for localhost only.", 500);
    }
  }
}
